-- 來源 https://github.com/yanhuacuo/98wubi-tables > http://98wb.ysepan.com/
-- 數字、金額大寫
-- 觸發前綴默認爲 recognizer/patterns/number 的第 2 個字符，即 R
...
local function decimal_func(str, posMap, valMap)
    local dec
    posMap = posMap or { [1] = "角", [2] = "分", [3] = "釐", [4] = "毫" }
    valMap = valMap or { [0] = "零", "壹", "貳", "叄", "肆", "伍", "陸", "柒", "捌", "玖" }
...
local function formatNum(num, t)
    ...
    if tonumber(t) < 1 then
        ...
        wordFigure = { "零", "壹", "貳", "叄", "肆", "伍", "陸", "柒", "捌", "玖" }
    end
...
-- 數值轉換爲中文
local function number2cnChar(num, flag, digitUnit, wordFigure) --flag=0中文小寫反之爲大寫
    local result = ""

    if tonumber(flag) < 1 then
        digitUnit = digitUnit or { [1] = "萬", [2] = "億" }
...
local function number_translatorFunc(num)
    ...
    if numberPart.dot ~= "" then
        ...
        --         "〔數字大寫〕" })
        local upperRaw = number2cnChar(numberPart.int, 1, { "萬", "億" }, { "〇", "一", "十", "點" })
    ...
    table.insert(result,
        ...
        decimal_func(numberPart.dec, { [1] = "角", [2] = "分", [3] = "釐", [4] = "毫" },
            { [0] = "〇", "一", "二", "三", "四", "五", "六", "七", "八", "九" }), "" })

    local number2cnCharInt = number2cnChar(numberPart.int, 1)
    local number2cnCharDec = decimal_func(numberPart.dec, { [1] = "角", [2] = "分", [3] = "釐", [4] = "毫" },
        { [0] = "零", "壹", "貳", "叄", "肆", "伍", "陸", "柒", "捌", "玖" })
    if string.len(numberPart.int) > 4 and number2cnCharInt:find('^拾[壹貳叄肆伍陸柒捌玖]?') and number2cnCharInt:find('[萬億]') then -- 簡易地規避 utf8 匹配問題
...
local function number_translator(input, seg, env)
    -- 獲取 recognizer/patterns/number 的第 2 個字符作爲觸發前綴
